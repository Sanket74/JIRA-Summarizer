        ...
        try:
            with urllib.request.urlopen(req, context=ctx) as response:
                ...
                if 15 <= word_count <= 20:
                    ...
                    print(f"[{row_idx}] Attempt {attempt+1}: Word count constraint violated ({word_count} words): {text}", flush=True)
                    data["generationConfig"]["temperature"] = min(0.9, data["generationConfig"]["temperature"] + 0.15)
                    req = urllib.request.Request(URL, data=json.dumps(data).encode('utf-8'), headers={'Content-Type': 'application/json'})
                    time.sleep(1.5)
        except urllib.error.HTTPError as e:
            if e.code in (429, 503):
                print(f"[{row_idx}] Rate limited or Overloaded ({e.code}). Retrying in {10 * (attempt + 1)} seconds...", flush=True)
                time.sleep(10 * (attempt + 1))
            else:
                print(f"[{row_idx}] HTTP Error {e.code}: {e.read().decode()}", flush=True)
                time.sleep(5)
        except Exception as e:
            print(f"[{row_idx}] Error: {e}. Retrying in 5 seconds...", flush=True)
            time.sleep(5)
...
def process_csv():
    ...
    def process_row(idx, row):
        # We process all rows to ensure they strictly follow the prompt since we might have leftover old format
        # If it's already a good format we can skip, but since we don't know, let's process all.
        # Let's see if we can detect the new format: no em-dashes and exactly 15-20 words, but wait
        # The user just asked to update it using the output from the prompt. We should just process all.
        print(f"Processing row {idx+1}/{len(rows)}...", flush=True)
        summary = generate_summary(row['body'], idx+1)
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        # We don't save per row, we wait for all to complete then save
        completed = 0
